...
export function WebsiteListStats({ websites }: WebsiteListStatsProps) {
  ...
  const onlineWebsites = websites.filter(
    website => website.status === 'online'
  ).length;
  const offlineWebsites = websites.filter(
    website => website.status === 'offline'
  ).length;
  const degradedWebsites = websites.filter(
    website => website.status === 'degraded'
  ).length;

  const avgUptime =
    websites.reduce((sum, website) => sum + website.uptime, 0) / totalWebsites;
  const avgResponseTime =
    websites
      .filter(website => website.status === 'online')
      .reduce((sum, website) => sum + website.responseTime, 0) / onlineWebsites;
